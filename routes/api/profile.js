...
router.get("/github/:username", (req, res) => {
  try {
    ...
    request(options, (error, response, body) => {
      ...
      // if we get a succesful response send the response
      //body will be aregular string so we need to surround it with JSON.parse
      res.json(JSON.parse(body));
    });
  } catch (err) {
    console.error("error--profile.js--/github/:username", err.message);
    res.status(500).send("Server Error");
  }
});
